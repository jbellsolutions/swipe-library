#!/usr/bin/env python3
"""Build a consolidated swipe file database for the local workspace."""
import glob
import json
import os
...
OUTPUT_DB = SWIPE_DIR / "swipe_database.json"
...
def build():
    ...
    for sender_file in sorted(glob.glob(str(IDS_DIR / "*.json"))):
        sender_path = Path(sender_file)
        sender_key = sender_path.stem

        with sender_path.open() as f:
            id_list = json.load(f)
        ...
        if content_dir.exists():
            for cf in glob.glob(str(content_dir / "*.json")):
                with open(cf) as f:
                    content = json.load(f)
    ...
    compact_path = SWIPE_DIR / "swipe_compact.json"
